...
def filter_by_character_position(words, char_positions):
    ...
    for word in words:
        match = all(
            len(word) > pos and word[pos] == char
...
def fetch_content_with_selenium_edge(url):
    """Fetch the HTML content of the website using Selenium with Edge."""
    options = Options()
        ...
        if specific_positions.strip():
            try:
                char_positions = {
                    int(pos): char
                    for pos, char in (
                        item.split(":") for item in specific_positions.split(",")
                    )
                }
    ...
    try:
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as file:
                existing_words = set(
                    file.read().split()
                )  # Read and split into a set of unique words
        except FileNotFoundError:
...
def main():
    ...
    if option == "1":
        ...
        try:
            ...
            if html_content is None:
                use_selenium = (
                    input("\nFetching failed. Do you want to try Selenium? (y/n): ")
                    .strip()
                    .lower()
                )
                if use_selenium == "y":
                    html_content = fetch_content_with_selenium_edge(url)
                else:
                    sys.exit()
    ...
    irregular_singular_plural_forms = (
        input(
            "\nDo you want to add singular, plural and irregular forms to word list? (yes/no): "
        )
        .strip()
        .lower()
    )
    if irregular_singular_plural_forms == "yes":
        try:
            ...
            print(
                "Failed to add singular, plural and irregular forms to word list. continue with original word list."
            )
    ...
    try:
        ...
        unique_char_words, duplicate_char_words = separate_words_by_unique_characters(
            words
        )
    ...
    if option == "2":
        update_textfile = (
            input(
                "\nDo you want to add fetched words from URL to the text file (data/text.txt)? (yes/no): "
            )
            .strip()
            .lower()
        )
